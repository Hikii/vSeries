...
namespace vSupport_Series.Champions
{
    public class Lux : Helper
    {
        ...
        private static void WManager()
        {
            ...
            foreach (var shield in ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsAlly && !x.IsMe && x.Distance(ObjectManager.Player.Position) < W.Range && !x.IsDead && x.IsVisible && !x.IsZombie))
            {
